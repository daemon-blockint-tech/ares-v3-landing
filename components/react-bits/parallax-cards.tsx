...
export interface ParallaxCardsProps {
  /** Array of image URLs to display (omit when using `cards`) */
  images?: string[];
  /** Text/icon cards instead of photos */
  cards?: ParallaxCardContent[];
  ...
  onCardClick?: (index: number, payload: string) => void;
...
const ParallaxCards = ({
  ...
  const contentMode = Boolean(cards?.length);
  const sourceLength = contentMode
    ? cards!.length
    : (images?.length ?? 0);
  const totalCards = Math.min(cardCount || sourceLength, 12);
  ...
  const handleCardClick = useCallback(
    (idx: number) => {
      ...
      onCardClick?.(
        idx,
        contentMode ? (cards![idx]?.title ?? "") : (images?.[idx] ?? ""),
      );
    },
    [
      focusedCardIndex,
  ...
  useEffect(() => {
    arrangeElements(focusedCardIndex);
  }, [images, cards, cardCount, arrangeElements, focusedCardIndex]);
          ...
          return (
            <button
              ...
              className={cn(
                ...
                contentMode
                  ? "flex cursor-pointer flex-col rounded-2xl border border-white/12 bg-gradient-to-b from-white/[0.09] to-white/[0.02] p-5 text-left backdrop-blur-md sm:p-6"
                  : "rounded-lg border-0 bg-cover bg-center cursor-pointer",
                "will-change-transform transition-opacity",
                "outline-none focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40",
                focusedCardIndex === idx && "ring-2 ring-white/40",
              )}
              style={{
                width: cardWidth ?? (contentMode ? 300 : "min(430px, 90vw)"),
                height: cardHeight ?? (contentMode ? 220 : "min(610px, 130vw)"),
                backgroundImage:
                  !contentMode && imageUrl ? `url(${imageUrl})` : undefined,
                transformStyle: "preserve-3d",
                pointerEvents: "auto",
                userSelect: "none",
              }}
            >
              {contentMode && card ? (
                <>
                  {card.icon ? (
                    <div className="mb-3 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/12 bg-white/[0.06]">
                      {card.icon}
                    </div>
                  ) : null}
                  <h3 className="text-[0.9375rem] font-medium leading-snug tracking-tight text-white">
                    {card.title}
                  </h3>
                  <p className="mt-2 line-clamp-5 text-xs leading-relaxed text-white/60 sm:text-[0.8125rem]">
                    {card.description}
                  </p>
                </>
              ) : null}
