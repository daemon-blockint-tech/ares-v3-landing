...
export interface MetallicSwirlProps {
  ...
  /** Gradient start color (hex), e.g. Solana purple */
  gradientFrom?: string;
  /** Gradient end color (hex), e.g. Solana green */
  gradientTo?: string;
...
void main() {
  ...
  float pulse = 0.5 + 0.5 * sin(val * 2.85 + t * 0.42);
  float sweep = 0.5 + 0.5 * sin(st.x * 1.4 + st.y * 0.9 + val * 1.1 - t * 0.18);
  float mixT = clamp(mix(pulse, sweep, 0.42) + 0.1 * sin(val * 4.5 - t * 0.22), 0.0, 1.0);
  vec3 col = mix(uColorA, uColorB, mixT);
  col *= uBright;

  col += cursorInfluence * 0.07 * uBright;

  float lum = dot(col, vec3(0.299, 0.587, 0.114));
  vec3 result = mix(uBg, col, clamp(lum * 3.6, 0.0, 1.0));
...
const MetallicSwirl: React.FC<MetallicSwirlProps> = ({
  ...
  const colorA = useMemo(() => parseHexColor(gradientFrom), [gradientFrom]);
  const colorB = useMemo(() => parseHexColor(gradientTo), [gradientTo]);
